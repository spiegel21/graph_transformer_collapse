...
from graphgps.loader.dataset.sbm import Fixed_SBM
...
def load_dataset_master(format, name, dataset_dir):
    ...
    # GraphGym default loader for Pytorch Geometric datasets
        ...
        if(name == 'Cora'):
            print('EDITING CORA MASKS')
            dataset[0].train_mask[:] = True
            dataset[0].val_mask[:] = True
            dataset[0].test_mask[:] = True
        ...
        if name == 'sbm':
            dataset = Fixed_SBM(root=dataset_dir)
    else:
